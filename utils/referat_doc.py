...
def create_referat_document(tema: str, sahifa: int, uslub: str, text: str, 
                        universitet: str, fakultet: str, muallif: str, 
                        kurs_guruh: str, doc_type: str = "REFERAT", plan: str = None, user_id: int = None) -> str: 
    """Word formatida dokument yaratish (Referat uchun sodda format)""" 
    try: 
        doc = Document() 
        ...
        # 1. TITUL VARAQ
        # --------------------------------------------------------- 
        def add_titul_para(content, bold=False, size=16, space_after=0): 
        ...
        label = "Ilmiy rahbar: " if "KURS" in doc_type.upper() else "Tekshirdi: " 
        ...
        if plan: 
            ...
            for line in plan_lines: 
                line = line.strip() 
                if not line or line.upper() == 'REJA': continue 
                
                # Kirish ichidagi mayda bandlarni (1, 2, 3...) mundarijadan olib tashlash 
        ...
        while i < len(lines):
            ...
            # --- JADVAL LOGIKASI (Sodda) ---
            # Referat uchun ham jadvallar kerak bo'lishi mumkin, shuning uchun qoldiramiz.
            ...
            if is_header:
                if clean_upper in seen_headers:
                    i += 1
                    continue
                seen_headers.append(clean_upper)
        ...
        folder_map = {
            "REFERAT": "referatlar",
            "KURS ISHI": "kurs_ishlari",
            "MUSTAQIL ISH": "mustaqil_ishlar"
        }
        ...
        logger.error(f"❌ Word dokumenti yaratishda xatolik: {str(e)}")
        return None
